...
$total_message_connect_form_db = mysqli_query($db_conntect,$total_messages_query);
$total_message = mysqli_fetch_assoc($total_message_connect_form_db);

$unread_messages_query = "SELECT COUNT(*) AS unread_message FROM visitor_messages WHERE status = 1";
$unread_message_connect_form_db = mysqli_query($db_conntect,$unread_messages_query);
$unread_message = mysqli_fetch_assoc($unread_message_connect_form_db);

$read_messages_query = "SELECT COUNT(*) AS read_message FROM visitor_messages WHERE status = 2";
$read_message_connect_form_db = mysqli_query($db_conntect,$read_messages_query);
$read_message = mysqli_fetch_assoc($read_message_connect_form_db);
